...
/**
 * 向量类实现的目的：
 * 提供基于数组的操作集合方法，并且采用synchronized 进行线程安全的处理
 * 包括：迭代器的迭代过程、迭代过程的操作都用上了线程安全
 * 但是基本功能与ArrayList相差无二。
 * @param <E>
 */
public class Vector<E>
    ...
    /**
     * 存储向量的分量所在的数组缓冲区
     * 向量的容量是这个数组缓冲区的长度,并且至少足够大到包含所有向量的元素。
     *
     * <p>向量中最后一个元素后面的任何数组元素都为空。
     */
     ...
     * 返回该向量的当前容量。
     *
     * @返回当前容量(其内部的长度) 数据数组，保存在字段{@code elementData}中
     * 这个向量)
     */
