 ...
 * 存在不同步的并发修改。  Fail-fast iterators
...
public class ArrayList<E> extends AbstractList<E>
    ...
    /**
     * 构造一个初始容量为10的空列表。默认创建就是这个，大小为10.
     */
    public ArrayList() {
    ...
    /**
     * 将此{@code ArrayList}实例的容量调整为
     * 列表的当前大小。应用程序可以使用此操作最小化
     * {@code ArrayList}实例的存储。
     * 作用：用于减少内存分配，对于内存小的，但是CPU性能好的可以使用
     */
    public void trimToSize() {
        modCount++;
        if (size < elementData.length) {
            //调用一个Array的复制方法，复制实际长度的数组。
            elementData = (size == 0)
                    ? EMPTY_ELEMENTDATA
    ...
    /**
     * 增加这个{@code ArrayList}实例的容量，如果
     * 必须，以确保它可以容纳至少数量的元素由最小容量参数指定。
     *
    ...
    public void ensureCapacity(int minCapacity) {
        if (minCapacity > elementData.length
            ...
            modCount++;
            //grow就是增长方法；也是用Arrays.copy的底层实现扩容。
            grow(minCapacity);
        }
    }
    ...
    /**
     * 实现：逐个遍历，返回索引值最低的元素下标。（第一次索引到的）
     * 找不到为-1；但是如果是空则会出现对应的null对象的下标
     */
    ...
    /**
     * $$$$$
     * 返回这个{@code ArrayList}实例的浅拷贝(不会复制元素本身。)
     *
